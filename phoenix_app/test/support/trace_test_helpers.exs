defmodule TraceTestHelpers do
  ...
  def create_test_context(trace_id, opts \\ []) do
    ...
    context = if Keyword.get(opts, :with_n8n_config, true) do
  ...
  def create_test_workflow(opts \\ []) do
    ...
    connections = if node_count > 1 do
    ...
    for event <- events do
      :telemetry.attach(
        ...
        fn event_name, measurements, metadata, {pid, test_ref} ->
          send(pid, {:trace_telemetry, test_ref, %{
            event: event_name,
  ...
  def analyze_trace_telemetry(events, expected_trace_id) do
    ...
    events_by_type = Enum.group_by(events, fn event ->
    ...
    %{
      ...
      events_by_type: Enum.into(events_by_type, %{}, fn {type, type_events} ->
        {type, length(type_events)}
      end),
      trace_ids_found: events 
