defmodule ComprehensiveTraceCoverageTest do
  ...
  describe "middleware trace coverage" do
    test "AgentCoordinationMiddleware includes trace ID" do
      ...
      {:ok, enhanced_context} =
        SelfSustaining.ReactorMiddleware.AgentCoordinationMiddleware.init(context)

      assert Map.get(enhanced_context, :trace_id) == trace_id

      middleware_state =
        Map.get(enhanced_context, SelfSustaining.ReactorMiddleware.AgentCoordinationMiddleware)
    ...
    test "TelemetryMiddleware trace ID generation and propagation" do
      ...
      # Should be same in test environment
      assert trace_id == otel_trace_id

      # Middleware state should include trace_id
      middleware_state =
        Map.get(enhanced_context, SelfSustaining.ReactorMiddleware.TelemetryMiddleware)
  ...
  describe "telemetry trace consistency" do
    test "all system components emit trace-consistent telemetry" do
      ...
      {:ok, _enhanced_context} =
        SelfSustaining.ReactorMiddleware.AgentCoordinationMiddleware.init(context)

      # 2. Web request
      _conn =
        Plug.Test.conn(:get, "/consistency-test", "")
        |> Plug.Test.put_req_header("x-trace-id", trace_id)
        |> SelfSustainingWeb.Plugs.TraceHeaderPlug.call([])
      ...
      {:ok, _step_result} =
        SelfSustaining.ReactorSteps.N8nWorkflowStep.run(arguments, test_context, [])
  ...
  describe "trace ID format validation" do
    test "all generated trace IDs follow consistent format" do
      ...
      generators = [
        fn ->
          {:ok, context} =
            SelfSustaining.ReactorMiddleware.TelemetryMiddleware.init(%{
        ...
        fn ->
          {:ok, context} =
            SelfSustaining.ReactorMiddleware.AgentCoordinationMiddleware.init(%{
              __reactor__: %{id: "test", steps: []}
            })
